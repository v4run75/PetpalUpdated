...
public class PreviewImage extends AppCompatActivity {
    Bundle bundle;
    ImageView imageView;
    ImageView remove, back;
    private static final int REQUEST_VIEW_DELETE_MEDIA = 6;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (bundle != null) {
            ...
            if (bundle.getString("type").equals("image")) {
                File f = new File(bundle.getString("path"));
                if (f.exists() && f.length() > 0)
                    Glide.with(this).load(f).into(imageView);
                Logger.d(" EXIF: ", ReadExif(bundle.getString("path")));

            } else if (bundle.getString("type").equals("galleryimage")) {
